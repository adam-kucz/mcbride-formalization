...
nth-var∉shift {elim}{m} k (var v) p
  ...
  where nth-k≠lift-k-old-v : ∀ k m (p : k < k + (m +1)) v →
        ...
        nth-k≠lift-k-old-v (k +1) m p (old v) q =
  ...
  nth-var∉shift k S (Id.coe (
    subrel $
      Het.ap3 (λ i (v : Var i)(t : Term i) → v ∈ fv t)
        (sym $ +-suc k m)
        (nth-var== (k<k+m+1 k m) (sym $ +-suc k m) (Id-refl k))
        (coe-eval coer S'))
    q)
...
nth-var∉shift {term}{m} k ([ π x: S ]→ T) _ | _ | ∨right q
  with ⟶ (∈bind (nth-var k (k<k+m+1 k m))
                  prevSafe
...
nth-var∉shift {term} {m} k ([ π x: S ]→ T) _
  ...
  Id.coe (
    ...
    Het.ap3 (λ i (v : Var i)(t : Term i) → v ∈ fv t)
      (sym $ +-suc (k +1) m)
      (nth-var== (k<k+m+1 (k +1) m) (sym $ +-suc (k +1) m) (Id-refl (k +1)))
      (proof coe coer T'
        het== T'
          :by: coe-eval coer T'
...
nth-var∉shift {term}{m} k (λx, t) p with ⟶ (∈bind v prevSafe (fv t')) p
  where v = nth-var k (postfix (_+ (m +1)) k)
        t' = rename (lift (lift-by k old)) t
